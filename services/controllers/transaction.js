...
router.put('/update-status/:id', async (req, res) => {
    ...
    try {
        ...
        const billingStatement = await billingCollection.findOne({bll_id: transaction.bill_id})
        ...
        const transactions = await transactionCollection.find({bill_id: billingStatement.bll_id}).toArray();

        const totalAmountOfAllTransactions = transactions.reduce((val, t) => t.trn_amount + val, 0)

        if(totalAmountOfAllTransactions >= billingStatement.bll_total_paid && totalAmountOfAllTransactions >= billingStatement.bll_total_amt_due) {
          await billingCollection.updateOne({bll_id:billingStatement.bll_id}, { $set: {
