...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if(!username || !password){
      return res.status(401).json({ error: 'failed to submit required parameters' });
    }
...
router.post('/create_property', async (req, res) => {
  ...
  try {
    ...
    const userWallet = await walletCollection.findOne({ wall_owner: homeownerID });
    if (!userWallet) {
      return res.status(404).json({ error: '*Wallet not found for the user' });
    }

    // Check if the property lot is already occupied
    const existingProperty = await propertiesCollection.findOne({ prop_lot_num: propertyLot });
    if (existingProperty) {
...
router.post('/wallet/spend', async (req, res) => {
  ...
  try {
    ...
    do {
      ...
      transactionExists = wallet.villwall_trn_hist.some(trn => trn.villwall_trn_id === transactionID);
...
router.get('/settings/misc/water_rate', async (req, res) => {
  try {
    ...
    const waterRates = await settingsCollection.find({ misc_type: "water_rate" }).toArray();
...
router.get('/settings/misc/garb_rate', async (req, res) => {
  try {
    ...
    // Find the document with misc_type equal to 'hoa_rate'
    const hoaRate = await settingsCollection.findOne({ misc_type: "garb_rate" });
...
router.post('/settings/delete-rate', async (req, res) => {
  ...
  try {
    ...
    if (result.deletedCount === 0) {
      return res.status(404).json({ error: 'Rate not found or already deleted' });
    }
